...
def _compute_and_store_results(d_syms, b_syms, library, p_thresh):
    ...
    parsed = overlaps["term"].apply(parse_enrichr_term)
    overlaps["term_stable_id"]  = parsed.apply(lambda t: t[0])
    overlaps["term_name_clean"] = parsed.apply(lambda t: t[1])
    overlaps["_id_key"]   = overlaps["term_stable_id"].astype(str).str.strip().str.upper()
    overlaps["_name_key"] = _norm(overlaps["term_name_clean"].fillna(overlaps["term"]))
    overlaps["_term_raw"] = overlaps["term"].astype(str).str.strip()

    overlaps["n_genes"] = overlaps["_id_key"].map(id2n)
    mask_missing = overlaps["n_genes"].isna()
    if mask_missing.any():
        overlaps.loc[mask_missing, "n_genes"] = overlaps.loc[mask_missing, "_name_key"].map(name2n)

    overlaps["parent_paths"] = overlaps["_id_key"].map(id2path)
    mask_p = overlaps["parent_paths"].isna()
    if mask_p.any():
        overlaps.loc[mask_p, "parent_paths"] = overlaps.loc[mask_p, "_name_key"].map(name2path)

    overlaps["avg_p_value"] = (overlaps["p_value_disease"] + overlaps["p_value_bhb"]) / 2.0
    overlaps["enriched_tissues"] = overlaps["_id_key"].map(enriched_tissues_by_id).fillna("")

    # Prepare sets for hub annotations/counters
    disease_lists = overlaps["overlap_genes_disease"].apply(parse_gene_list)
    if "overlap_genes_bhb_annot" not in overlaps.columns:
        overlaps["overlap_genes_bhb_annot"] = overlaps["overlap_genes_bhb"]
    ...
    for raw_term, sid, namek, dis_genes, bhb_annot in zip(
        overlaps["_term_raw"], overlaps["term_stable_id"], overlaps["_name_key"],
        disease_lists, overlaps["overlap_genes_bhb_annot"]
    ):
        hs = _hub_set_for_row(raw_term, sid, namek)
        hub_sets.append(hs)
        dis_set = set([g.strip().upper() for g in dis_genes])
        bhb_set = set([g.replace("*","").strip().upper() for g in str(bhb_annot or "").split(";") if g.strip()])
    ...
    overlaps["_hub_set"] = hub_sets
    overlaps["disease_hub_overlap"] = disease_hub_overlap
    overlaps["bhb_hub_overlap"] = bhb_hub_overlap
    overlaps["overlapping_hubs"] = overlapping_hubs
    overlaps["hub_genes_term_annot"] = hub_term_annots
...
if st.session_state.results_ready and st.session_state.overlaps_raw is not None:
    overlaps = st.session_state.overlaps_raw.copy()

    st.subheader("Overlapping Reactome terms")
    st.caption("Terms where **both** gene sets have raw p-value below the selected threshold.")
    st.write(f"Found **{len(overlaps)}** overlapping terms at p < {P_THRESH}.")

    # ---- Size filter slider ----
    if overlaps["n_genes"].notna().any():
        values = pd.to_numeric(overlaps["n_genes"], errors="coerce").dropna().astype(int)
        ...
        if cur_min < cur_max:
            ...
            overlaps = overlaps[
                overlaps["n_genes"].fillna(cur_max + 1).between(size_range[0], size_range[1])
            ]
    else:
        st.info("Term sizes unavailable or single-valued; size filter disabled.")

    # ---- Tissue filter ----
    available_tissues = sorted({t.strip() for val in overlaps["enriched_tissues"] if isinstance(val, str) for t in val.split(";") if t.strip()})
    ...
    if active_tissues:
        mask = overlaps["enriched_tissues"].apply(
            lambda s: any(t in (s or "") for t in active_tissues)
        )
        overlaps = overlaps[mask]

    # ---- Sorting options (incl. BHB overlap & BHB hub overlap) ----
    ...
    csv_bytes = overlaps.to_csv(index=False).encode("utf-8")
    ...
    for row in overlaps.itertuples(index=False):
        ...
        if title_id:
            header_visible = f"{title_name} — {gene_label}, avg p={avgp_str}, {bhb_label}, {hub_label}"
        else:
            header_visible = f"{title_name} — {gene_label}, avg p={avgp_str}, {bhb_label}, {hub_label}"
